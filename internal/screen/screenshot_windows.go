...
func Capture(x, y, width, height int) (*image.RGBA, error) {
	...
	if !win.BitBlt(memory_device, 0, 0, int32(width), int32(height), hdc, int32(x), int32(y), win.SRCCOPY|win.CAPTUREBLT) {
...
func getDesktopWindow() win.HWND {
	ret, _, _ := syscall.SyscallN(funcGetDesktopWindow, 0, 0, 0, 0)
	return win.HWND(ret)
}

func enumDisplayMonitors(hdc win.HDC, lprcClip *win.RECT, lpfnEnum uintptr, dwData uintptr) bool {
	ret, _, _ := syscall.SyscallN(funcEnumDisplayMonitors, 4,
		uintptr(hdc),
...
func getMonitorRealSize(hMonitor win.HMONITOR) *win.RECT {
	...
	ret, _, _ := syscall.SyscallN(funcGetMonitorInfo, 2, uintptr(hMonitor), uintptr(unsafe.Pointer(&info)), 0)
	if ret == 0 {
		return nil
	}
	...
	if ret, _, _ := syscall.SyscallN(funcEnumDisplaySettings, 3, uintptr(unsafe.Pointer(&info.DeviceName[0])), _ENUM_CURRENT_SETTINGS, uintptr(unsafe.Pointer(&devMode))); ret == 0 {
		return nil
	}
